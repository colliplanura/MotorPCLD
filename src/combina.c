...
void combina(int gi, reducao *reducoes, float acmAmtr, float acmPcld,
		float *melhorIndice, char resultAnt[], resultado *resultados) {
	int fi;
	char resultTemp[39] = "";

	for (gi; gi < (*reducoes).qtGrupos; gi++) {
		for (fi = 0; fi < (*reducoes).grupos[gi].qtFaixas; fi++) {
			sprintf(resultTemp, "%s%02i%i+", resultAnt, gi + 1, fi + 1);
			...
			if (gi < ((*reducoes).qtGrupos - 1)) {
				combina(gi + 1, reducoes, acmAmtr, acmPcld, melhorIndice,
...
int main(int argc, char *argv[]) {
	int maxFaixas, g, f, amtrIni, pcldIni, pcldFim;
	// char resultTemp[39] = "";
	...
	printf("Grupos: %i | Máximo de Faixas: %i | Melhor Indice: %f\n\n",
			reducoes.qtGrupos, maxFaixas, melhorIndice);

	for (g = 0; g < reducoes.qtGrupos; g++) {
		...
		for (f = 0; f < reducoes.grupos[g].qtFaixas; f++) {
			...
			printf("%5i%1i: %5.f %5.f", g + 1, f + 1,
					reducoes.grupos[g].faixas[f].amtr,
					reducoes.grupos[g].faixas[f].pcld);
		}
		printf("\n");
	}
	...
	combina(0, &reducoes, 0, 0, &melhorIndice, "", &resultados);
