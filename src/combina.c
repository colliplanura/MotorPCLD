...
erro validaEntrada(rqsc requisicao) {
	...
	if ((requisicao.qtGrupos == 0) || (requisicao.qtGrupos > MAX_GRUPOS)) {
		retErro.seqlErro = 1;
		sprintf(retErro.txErro, "Quantidade de grupos não está entre 1 e %i.", MAX_GRUPOS);
		return retErro;
	}

	for (g = 0; g < requisicao.qtGrupos; g++) {

		if ((requisicao.grupos[g].qtFaixas == 0) || (requisicao.grupos[g].qtFaixas > MAX_FAIXAS)) {
			retErro.seqlErro = 2;
			sprintf(retErro.txErro, "Quantidade de faixas não está entre 1 e %i. Grupo: %i.", MAX_FAIXAS, g);
			return retErro;
		}
		...
		for (f = 0; f < requisicao.grupos[g].qtFaixas; f++){
			...
			if (requisicao.grupos[g].faixas[f].fxa == 0) {
				retErro.seqlErro = 3;
				sprintf(retErro.txErro, "Número da faixa não informado. Grupo: %i. Faixa: %i.", g, f);
				return retErro;
			}

			if (requisicao.grupos[g].faixas[f].amtr <= 0) {
				retErro.seqlErro = 4;
				sprintf(retErro.txErro, "Valor da amortização não informado. Grupo: %i. Faixa: %i.", g, f);
				return retErro;
			}

			if (requisicao.grupos[g].faixas[f].pcld <= 0) {
				retErro.seqlErro = 5;
				sprintf(retErro.txErro, "Valor da reversão da PCLD não informado. Grupo: %i. Faixa: %i.", g, f);
				return retErro;
			}

			if (requisicao.grupos[g].faixas[f].pcld < requisicao.grupos[g].faixas[f].amtr) {
				retErro.seqlErro = 6;
				sprintf(retErro.txErro, "Dados inconsistentes. Valor da reversão da PCLD menor que amortização. Grupo: %i. Faixa: %i.", g, f);
				return retErro;
			}

			if (f > 0) {
				if (requisicao.grupos[g].faixas[f].pcld < requisicao.grupos[g].faixas[f - 1].pcld) {
					retErro.seqlErro = 7;
					sprintf(retErro.txErro, "Dados inconsistentes. Valor da reversão da PCLD não é crescente por faixas. Grupo: %i. Faixa Atual: %i. Faixa anterior: %i", g, f, f - 1);
					return retErro;
				}

				if (requisicao.grupos[g].faixas[f].amtr < requisicao.grupos[g].faixas[f - 1].amtr) {
					retErro.seqlErro = 8;
					sprintf(retErro.txErro, "Dados inconsistentes. Valor da amortização não é crescente por faixas. Grupo: %i. Faixa Atual: %i. Faixa anterior: %i", g, f, f - 1);
					return retErro;
				}
			}
		}
...
double menorAmtr(rqsc *requisicao, rstd *result1) {
	...
	for (g = 0; g < (*requisicao).qtGrupos; g++) {
		if  ((*requisicao).grupos[g].faixas[0].amtr < menorAmtr) {
			...
			(*result1).grFxa[0].fxa = (*requisicao).grupos[g].faixas[0].fxa;
...
double maiorAmtr(rqsc *requisicao, rstd *result2) {
	...
	for (g = 0; g < (*requisicao).qtGrupos; g++) {
		maiorAmtr += (*requisicao).grupos[g].faixas[(*requisicao).grupos[g].qtFaixas - 1].amtr;
		(*result2).grFxa[(*result2).qtGrFxa].gr = (*requisicao).grupos[g].gr;
		(*result2).grFxa[(*result2).qtGrFxa].fxa = (*requisicao).grupos[g].faixas[(*requisicao).grupos[g].qtFaixas - 1].fxa;
		(*result2).qtGrFxa++;
	}
...
void combina(rqsc *requisicao, rpst *resposta, int gi, double acmAmtr, double acmPcld, double mediaAmtr, rstd resultAtu) {
	int fi;
	...
	for (gi; gi < (*requisicao).qtGrupos; gi++) {
		for (fi = 0; fi < (*requisicao).grupos[gi].qtFaixas; fi++) {
			...
			resultAtu.grFxa[resultAtu.qtGrFxa].gr = (*requisicao).grupos[gi].gr;
			resultAtu.grFxa[resultAtu.qtGrFxa].fxa = (*requisicao).grupos[gi].faixas[fi].fxa;
			resultAtu.qtGrFxa++;
			...
			/*
			 * Resultado 4 - Melhor Índice entre -10% e +10% da média da amortizacao
             */
			if ( (acmAmtr > (mediaAmtr * 0.9)) && (acmAmtr < (mediaAmtr * 1.1)) ) {
			...
			/*
			 * Resultado 6 - Melhor reversão da PCLD com a 80% da entrada
             */
			if ((*requisicao).vlEntrada > 0) {
			...
			/*
			 * Resultado 7 - Melhor reversão da PCLD com a 90% da entrada
             */
			if ((*requisicao).vlEntrada > 0) {
			...
			/*
			 * Resultado 8 - Melhor reversão da PCLD com a 110% da entrada
             */
			if ((*requisicao).vlEntrada > 0) {
			...
			/*
			 * Resultado 9 - Melhor reversão da PCLD com a 120% da entrada
             */
			if ((*requisicao).vlEntrada > 0) {
			...
			if (gi < ((*requisicao).qtGrupos - 1)) {
				combina(requisicao, resposta, gi + 1, acmAmtr, acmPcld, mediaAmtr, resultAtu);
			}
		}
	}
	return;
...
int main(parm *parametro) {
	...
	vlMenorAmtr = menorAmtr(&(*parametro).requisicao, &(*parametro).resposta.result[1]);

	(*parametro).resposta.qtRstd = 2;
	vlMaiorAmtr = maiorAmtr(&(*parametro).requisicao, &(*parametro).resposta.result[2]);
	...
	result.qtGrFxa = 0;
	combina(&(*parametro).requisicao, &(*parametro).resposta, 0, 0, 0, (vlMenorAmtr + vlMenorAmtr) / 2, result);
